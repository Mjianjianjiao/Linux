...
int main(int argc, char* argv[]){
  ...
  epoll_ctl(epollfd, EPOLL_CTL_ADD, listenfd, &events);

  struct epoll_event suc_events;
  while(1){
    int nfds = epoll_wait(epollfd, &suc_events, 1000, 3000);
    if(nfds < 0){
    ...
    for(int i = 0; i < nfds; i++){
      if(suc_events.data.fd == listenfd){
        ...
        epoll_ctl(epollfd, EPOLL_CTL_ADD, cli_fd, &events);
      }
      else if(events.events & EPOLLIN){
        //不是监听描述符，且时一个可读事件
        char buf[1024];
        int recv_len = recv(events.data.fd, buf, 1024, 0);
        if(recv_len < 0){
        ...
        printf("[%s : %d] : %s \n", ip.c_str(), port, buf);
        string resp = "hello";
        send(events.data.fd, resp.c_str(), resp.size(), 0);
      }
    }
  }
